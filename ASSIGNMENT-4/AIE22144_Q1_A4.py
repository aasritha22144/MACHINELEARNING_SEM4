...
class_a_data = file[file['Title'].str.contains('Crispy Salt and Pepper Potatoes', na=False)].copy()
class_b_data = file[file['Title'].str.contains('Turmeric Hot Toddy', na=False)].copy()
...
class_a_data['Cleaned_Ingredients'] = class_a_data['Ingredients'].apply(clean_and_tokenize)
class_b_data['Cleaned_Ingredients'] = class_b_data['Ingredients'].apply(clean_and_tokenize)
if len(class_a_data) > 1:
    class_a_cov_matrix = np.cov(class_a_data['Cleaned_Ingredients'].apply(len), rowvar=False)
    class_a_intraclass_spread = np.trace(class_a_cov_matrix)
else:
    class_a_intraclass_spread = np.nan

if len(class_b_data) > 1:
    class_b_cov_matrix = np.cov(class_b_data['Cleaned_Ingredients'].apply(len), rowvar=False)
    class_b_intraclass_spread = np.trace(class_b_cov_matrix)
else:
    class_b_intraclass_spread = np.nan

class_a_mean_vector = np.mean(class_a_data['Cleaned_Ingredients'].apply(len), axis=0)
class_b_mean_vector = np.mean(class_b_data['Cleaned_Ingredients'].apply(len), axis=0)
